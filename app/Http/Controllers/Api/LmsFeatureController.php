...
class LmsFeatureController extends Controller
{
    ...
    {
        return DB::table('courses')->leftJoin('subjects', 'subjects.id', '=', 'courses.subject_id')
            ->where('courses.is_published', true)
            ->when($request->q, fn ($query, $value) => $query->where(fn ($search) => $search->where('courses.title', 'like', '%'.$value.'%')->orWhere('courses.description', 'like', '%'.$value.'%')))
            ->select('courses.*', 'subjects.name as subject')
            ->selectSub(fn ($query) => $query->from('lessons')->selectRaw('count(*)')->whereColumn('lessons.course_id', 'courses.id')->where('is_published', true), 'lessons_count')
            ->orderBy('courses.title')->paginate(12);
    }

    public function publicCourse(int $id)
    {
        $course = DB::table('courses')->leftJoin('subjects', 'subjects.id', '=', 'courses.subject_id')
            ->where('courses.id', $id)->where('courses.is_published', true)
            ->select('courses.*', 'subjects.name as subject')->first();
        abort_unless($course, 404);
        $course->lessons = DB::table('lessons')->where('course_id', $id)->where('is_published', true)
            ->orderBy('position')->get(['id', 'title', 'summary', 'duration_minutes', 'position']);
    ...
    public function courses(Request $request)
    {
        $query = DB::table('courses')->leftJoin('subjects', 'subjects.id', '=', 'courses.subject_id')
            ...
            ->when(!$request->user()->hasRole('admin','teacher'),fn($q)=>$q->where('courses.is_published',true))
            ->when($request->user()->hasRole('teacher'),fn($q)=>$q->where('courses.created_by',$request->user()->id))
            ->select('courses.*', 'subjects.name as subject', 'course_enrollments.progress', 'course_enrollments.completed_at')
    ...
    {
        $this->courseStaff($request); $d=$request->validate(['title'=>['required','string','max:255'],'description'=>['nullable','string'],'subject_id'=>['nullable','exists:subjects,id'],'grade_level'=>['nullable','string','max:50'],'difficulty'=>['required',Rule::in(['beginner','intermediate','advanced'])],'price_lyd'=>['nullable','integer','min:0'],'course_code'=>['nullable','string','max:50','unique:courses,course_code'],'learning_objectives'=>['nullable','string'],'requirements'=>['nullable','string'],'language'=>['nullable',Rule::in(['ar','en','both'])],'thumbnail_path'=>['nullable','string','max:500'],'is_published'=>['boolean'],'lessons'=>['sometimes','array'],'lessons.*.title'=>['required','string','max:255'],'lessons.*.summary'=>['nullable','string'],'lessons.*.content'=>['nullable','string'],'lessons.*.video_url'=>['nullable','url','max:500'],'lessons.*.duration_minutes'=>['nullable','integer','min:0','max:1440']]);
        return DB::transaction(function()use($request,$d){$id=DB::table('courses')->insertGetId(['created_by'=>$request->user()->id,'subject_id'=>$d['subject_id']??null,'title'=>$d['title'],'slug'=>Str::slug($d['title']).'-'.Str::lower(Str::random(6)),'description'=>$d['description']??null,'grade_level'=>$d['grade_level']??null,'difficulty'=>$d['difficulty'],'price_lyd'=>$d['price_lyd']??0,'course_code'=>$d['course_code']??'MEH-'.date('Y').'-'.Str::upper(Str::random(6)),'learning_objectives'=>$d['learning_objectives']??null,'requirements'=>$d['requirements']??null,'language'=>$d['language']??'ar','thumbnail_path'=>$d['thumbnail_path']??null,'status'=>($d['is_published']??false)?'published':'draft','is_published'=>$d['is_published']??false,'created_at'=>now(),'updated_at'=>now()]);foreach($d['lessons']??[] as $i=>$lesson)DB::table('lessons')->insert($lesson+['course_id'=>$id,'position'=>$i+1,'is_published'=>$d['is_published']??false,'created_at'=>now(),'updated_at'=>now()]);return response()->json(DB::table('courses')->find($id),201);});
    }

    public function updateCourse(Request $request,int $id){$this->courseStaff($request);$course=DB::table('courses')->find($id);abort_unless($course,404);abort_if($request->user()->hasRole('teacher')&&(int)$course->created_by!==$request->user()->id,403);$d=$request->validate(['title'=>['sometimes','string','max:255'],'description'=>['nullable','string'],'grade_level'=>['nullable','string','max:50'],'difficulty'=>['sometimes',Rule::in(['beginner','intermediate','advanced'])],'price_lyd'=>['nullable','integer','min:0'],'learning_objectives'=>['nullable','string'],'requirements'=>['nullable','string'],'language'=>['nullable',Rule::in(['ar','en','both'])],'thumbnail_path'=>['nullable','string','max:500'],'is_published'=>['boolean']]);if(array_key_exists('is_published',$d))$d['status']=$d['is_published']?'published':'draft';DB::table('courses')->where('id',$id)->update($d+['updated_at'=>now()]);return DB::table('courses')->find($id);}
    public function destroyCourse(Request $request,int $id){$this->courseStaff($request);$course=DB::table('courses')->find($id);abort_unless($course,404);abort_if($request->user()->hasRole('teacher')&&(int)$course->created_by!==$request->user()->id,403);DB::table('courses')->where('id',$id)->update(['is_published'=>false,'status'=>'archived','updated_at'=>now()]);return response()->noContent();}
    public function storeLesson(Request $request,int $id){$this->courseStaff($request);$course=DB::table('courses')->find($id);abort_unless($course,404);abort_if($request->user()->hasRole('teacher')&&(int)$course->created_by!==$request->user()->id,403);$d=$request->validate(['title'=>['required','string','max:255'],'summary'=>['nullable','string'],'content'=>['nullable','string'],'video_url'=>['nullable','url','max:500'],'duration_minutes'=>['nullable','integer','min:0','max:1440'],'is_published'=>['boolean']]);$id2=DB::table('lessons')->insertGetId($d+['course_id'=>$id,'position'=>DB::table('lessons')->where('course_id',$id)->max('position')+1,'created_at'=>now(),'updated_at'=>now()]);return response()->json(DB::table('lessons')->find($id2),201);}
    ...
    public function registerForCourse(Request $request, int $id)
    {
        $course = DB::table('courses')->where(['id' => $id, 'is_published' => true])->first();
        abort_unless($course, 404);
        $data = $request->validate(['student_note' => ['nullable', 'string', 'max:2000']]);
        $status = (int) ($course->price_lyd ?? 0) === 0 ? 'approved' : 'pending_payment';
        $registrationId = DB::transaction(function () use ($request, $course, $data, $status) {
            DB::table('course_registrations')->updateOrInsert(
                ['course_id' => $course->id, 'user_id' => $request->user()->id],
                ['amount_lyd' => $course->price_lyd ?? 0, 'student_note' => $data['student_note'] ?? null, 'status' => $status, 'updated_at' => now(), 'created_at' => now()]
            );
            $registration = DB::table('course_registrations')->where(['course_id' => $course->id, 'user_id' => $request->user()->id])->first();
            if ($status === 'approved') {
                DB::table('course_enrollments')->updateOrInsert(['course_id' => $course->id, 'user_id' => $request->user()->id], ['started_at' => now(), 'updated_at' => now(), 'created_at' => now()]);
            }
            return $registration->id;
        });
        return response()->json(['id' => $registrationId, 'status' => $status, 'message' => $status === 'approved' ? 'Registration approved. Your course is ready.' : 'Registration created. Please upload payment proof.'], 201);
    }

    public function courseRegistrations(Request $request)
    {
        return DB::table('course_registrations')->join('courses', 'courses.id', '=', 'course_registrations.course_id')
            ->where('course_registrations.user_id', $request->user()->id)
            ->leftJoin('payment_proofs', 'payment_proofs.course_registration_id', '=', 'course_registrations.id')
            ->latest('course_registrations.created_at')->get(['course_registrations.*', 'courses.title as course_title', 'courses.course_code', 'payment_proofs.original_name as proof_name']);
    }

    public function uploadPaymentProof(Request $request, int $registration)
    {
        $row = DB::table('course_registrations')->where(['id' => $registration, 'user_id' => $request->user()->id])->first();
        abort_unless($row, 404);
        abort_if($row->status === 'approved', 409, 'This registration is already approved.');
        $file = $request->validate(['proof' => ['required', 'file', 'mimes:jpg,jpeg,png,pdf', 'max:5120']])['proof'];
        $path = $file->store('payment-proofs/'.$request->user()->id, 'local');
        DB::transaction(function () use ($registration, $file, $path) {
            if ($old = DB::table('payment_proofs')->where('course_registration_id', $registration)->first()) Storage::disk('local')->delete($old->file_path);
            DB::table('payment_proofs')->updateOrInsert(['course_registration_id' => $registration], ['file_path' => $path, 'original_name' => $file->getClientOriginalName(), 'mime_type' => $file->getMimeType(), 'file_size' => $file->getSize(), 'updated_at' => now(), 'created_at' => now()]);
            DB::table('course_registrations')->where('id', $registration)->update(['status' => 'pending_review', 'updated_at' => now()]);
        });
        return response()->json(['status' => 'pending_review', 'message' => 'Payment proof submitted for management review.'], 201);
    }

    public function adminCourseRegistrations(Request $request)
    {
        $this->admin($request);
        return DB::table('course_registrations')->join('courses', 'courses.id', '=', 'course_registrations.course_id')->join('users', 'users.id', '=', 'course_registrations.user_id')
            ->leftJoin('payment_proofs', 'payment_proofs.course_registration_id', '=', 'course_registrations.id')
            ->when($request->status, fn ($q, $status) => $q->where('course_registrations.status', $status))
            ->latest('course_registrations.created_at')->paginate(30, ['course_registrations.*', 'courses.title as course_title', 'users.name as student_name', 'users.email as student_email', 'payment_proofs.original_name as proof_name']);
    }

    public function reviewCourseRegistration(Request $request, int $registration)
    {
        $this->admin($request);
        $data = $request->validate(['status' => ['required', Rule::in(['approved', 'rejected'])], 'management_note' => ['nullable', 'string', 'max:2000']]);
        $row = DB::table('course_registrations')->find($registration); abort_unless($row, 404);
        DB::transaction(function () use ($request, $row, $data) {
            DB::table('course_registrations')->where('id', $row->id)->update(['status' => $data['status'], 'management_note' => $data['management_note'] ?? null, 'reviewed_by' => $request->user()->id, 'reviewed_at' => now(), 'updated_at' => now()]);
            if ($data['status'] === 'approved') DB::table('course_enrollments')->updateOrInsert(['course_id' => $row->course_id, 'user_id' => $row->user_id], ['started_at' => now(), 'updated_at' => now(), 'created_at' => now()]);
            else DB::table('course_enrollments')->where(['course_id' => $row->course_id, 'user_id' => $row->user_id])->delete();
        });
        return ['status' => $data['status'], 'message' => 'Registration review saved.'];
    }
    ...
    public function google(Request $request)
    {
        ...
        if(($google['aud']??null)!==config('services.google.client_id')||($google['email_verified']??'false')!=='true')throw ValidationException::withMessages(['credential'=>['This Google credential is not valid for MISR.']]);
    ...
    private function courseStaff(Request $request):void{abort_unless($request->user()->hasRole('admin','teacher'),403);}
