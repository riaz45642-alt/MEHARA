...
const META = {
  courses: ['professional','My Learning','Continue your courses and track lesson progress.','▤'],
  workbooks: ['professional','Learning Resources','Guides and resources for structured practice.','▥'],
  quizzes: ['professional','Assessments','Check understanding and prepare for applied work.','✓'],
  messages: ['professional','Messages','Conversations with your learning team.','✉'],
  notifications: ['professional','Notifications','Important updates from across the platform.','◉'],
  calendar: ['professional','Learning Calendar','Live classes, mentorship sessions and deadlines.','□'],
  certificates: ['professional','My Certificates','Verified records of completed learning.','◇'],
  saved: ['professional','Saved Learning','Courses and resources saved for later.','☆'],
  admin: ['professional','People & Roles','Manage platform access and responsibilities.','◎'],
  classes: ['professional','Cohorts & Classes','Manage learning groups and instructor access.','▦'],
  subjects: ['professional','Skills & Categories','Organize the platform learning catalogue.','⌘'],
  billing: ['professional','Learning Plans','Clear pricing with payment integration planned for Phase 2.','LYD'],
  bundles: ['professional','Project Toolkits','Curated resources for focused applied learning.','▣'],
}

function State({ loading, error, empty, retry }) {
  if (loading) return <div className="lms-state misr-state-skeleton" role="status" aria-label="Loading content"><span/><span/><span/></div>
  if (error) return <div className="lms-state error" role="alert"><strong>We could not load this content.</strong><span>Please try again. Technical details have been kept out of this view.</span><button onClick={retry}>Try again</button></div>
  if (empty) return <div className="lms-state"><span className="state-face">◇</span><strong>Your next step will appear here.</strong><span>Explore available learning or check back when new content is published.</span></div>
  return null
}
...
export default function ConnectedLms({ type, id, savedKind='bookmark' }) {
  ...
  return <main className={`connected-lms world-${meta[0]}`}>
    <section className="lms-hero"><div><span className="lms-eyebrow">MISR learning platform</span><h1>{meta[1]}</h1><p>{meta[2]}</p></div><div className="lms-mascot" aria-hidden="true">{meta[3]}</div></section>
    ...
    {!loading&&!error&&<section className="adventure-grid">
      ...
      {!['admin','messages','billing'].includes(type)&&visible.map((x,i)=><article className="adventure-card" key={x.id||i}>
        <div className="card-sticker" aria-hidden="true">{meta[3]}</div><div className="card-copy"><span className="card-kicker">{x.subject||x.event_type||x.content_type||x.role||'MISR'}</span><h2>{x.title||x.name||x.course_title||x.subject||`${x.action||x.kind} ${x.content_type||''}`}</h2>
...
function Detail({type,data,loading,error,retry}){
  const {api}=useApp();const [result,setResult]=useState(null),[answers,setAnswers]=useState({}),[busy,setBusy]=useState(false),[local,setLocal]=useState(data)
  ...
  return <main className={`connected-lms world-${type==='quizzes'?'dino':type==='courses'?'fox':'panda'}`}><section className="detail-banner"><button onClick={()=>history.back()}>← Back</button><h1>{local.title}</h1><p>{local.description||local.summary}</p>{local.progress!=null&&<div className="kid-progress"><span style={{width:`${local.progress}%`}}/></div>}</section>
...
function UserCard({item,api,onDone}){const [busy,setBusy]=useState(false);const save=async field=>{setBusy(true);try{await api.patch(`/admin/users/${item.id}`,field);onDone()}finally{setBusy(false)}};return <article className="adventure-card"><div className="avatar-dot">{item.name?.[0]}</div><div className="card-copy"><h2>{item.name}</h2><p>{item.email}</p><label>Access level<select disabled={busy} value={item.role} onChange={e=>save({role:e.target.value})}>{['student','teacher','admin'].map(r=><option value={r} key={r}>{r==='student'?'Learner':r==='teacher'?'Instructor':'Administrator'}</option>)}</select></label><button disabled={busy} onClick={()=>save({status:item.status==='active'?'suspended':'active'})}>{item.status==='active'?'Suspend':'Reactivate'}</button></div></article>}
